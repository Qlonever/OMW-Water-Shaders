...
vec3 doWaterSpecularLighting(vec2 screenCoord, vec3 viewPos, vec3 viewNormal, float dampen) {
    vec3 specular = vec3(0.0);
    vec3 viewDir = normalize(viewPos);
    float shininess = 50.0;
    ...
    for (uint i = 0u; i < grid.count; ++i) {
        ...
        vec3 lightWorldPos = (gl_ModelViewMatrixInverse * vec4(light.position.xyz,1)).xyz;

        if (lightWorldPos.z * cameraPos.z > 0) {
            vec3 lightPos = light.position.xyz - viewPos;
            float lightDistance = length(lightPos);
            vec3 lightDir = lightPos / lightDistance;
            float attenuation = calcAttenuation(light, lightDistance) * clusterFade(viewPos, light.radius);
